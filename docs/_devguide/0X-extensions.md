---
title: "Extensions"
permalink: /devguide/extensions/
excerpt: "Extend Emc4j with new extensions"
...
## Extensions
Minimal Mistakes has been developed as a [Gem-based theme](http://jekyllrb.com/devguide/themes/) for easier use, and 100% compatible with GitHub Pages when used as a remote theme.
...
[^structure]: See [**Structure** page]({{ "/devguide/structure/" | relative_url }}) for a list of theme files and what they do.
...
[ci-jekyll]: https://jekyllrb.com/devguide/deployment/automated/#continuous-integration-service
...
**Note:** If forking the theme be sure to update `Gemfile` as well. The one found at the root of the project is for building the theme's Ruby gem and is missing dependencies. To properly setup a [`Gemfile`](https://github.com/mmistakes/minimal-mistakes/blob/master/devguide/Gemfile) with the theme, consult the "[Install Dependencies](https://mmistakes.github.io/minimal-mistakes/devguide/installation/#install-dependencies)" section.
{: .notice--warning}
...
Starting with an empty folder and `Gemfile` you'll need to copy or re-create this [default `_config.yml`](https://github.com/mmistakes/minimal-mistakes/blob/master/_config.yml) file. For a full explanation of every setting be sure to read the [**Configuration**]({{ "/devguide/configuration/" | relative_url }}) section.
...
- [`_data/ui-text.yml`][ui-text.yml] - UI text [documentation]({{ "/devguide/ui-text/" | relative_url }})
- [`_data/navigation.yml`][navigation.yml] - navigation [documentation]({{ "/devguide/navigation/" | relative_url }})
...
- Replace `<site root>/index.md` with a modified [Minimal Mistakes `index.html`](https://github.com/mmistakes/minimal-mistakes/blob/master/index.html). Be sure to enable pagination if using the [`home` layout]({{ "/devguide/layouts/#home-page" | relative_url }}) by adding the necessary lines to **_config.yml**.
...
You won't need these anymore as they're bundled with the theme gem --- unless you intend to [override them](https://jekyllrb.com/devguide/themes/#overriding-theme-defaults).
...
If you customized any of these files leave them alone, and only remove the untouched ones. If done correctly your modified versions should [override](https://jekyllrb.com/devguide/themes/#overriding-theme-defaults) the versions bundled with the theme and be used by Jekyll instead.
...
**v4 Breaking Change:** Paths for image headers, overlays, teasers, [galleries]({{ "/devguide/helpers/#gallery" | relative_url }}), and [feature rows]({{ "/devguide/helpers/#feature-row" | relative_url }}) have changed and now require a full path. Instead of just `image: filename.jpg` you'll need to use the full path eg: `image: /assets/images/filename.jpg`. The preferred location is now `/assets/images/` but can be placed elsewhere or externally hosted. This applies to image references in `_config.yml` and `author.yml` as well.
{: .notice--danger}
